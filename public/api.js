  ...
  constructor(seed = null, slowMode = false, possibleDuplicateEvent = false) {
    ...
    that.intervalID = setInterval(() => {
      ...
      for (let index = 0; index < eventCount; index++) {
        /** @type {APIEventType} */
        let eventType = that.random() > 0.5 ? API_EVENT_TYPE.MESSAGE : that.random() > 0.5 ? API_EVENT_TYPE.GIFT : API_EVENT_TYPE.ANIMATED_GIFT;
        ...
        if (eventType == API_EVENT_TYPE.MESSAGE) {
          ...
          const randomGiftEmoji = that._sample(eventType == API_EVENT_TYPE.ANIMATED_GIFT ? ANIMATED_GIFT_OPTIONS : GIFT_OPTIONS);
